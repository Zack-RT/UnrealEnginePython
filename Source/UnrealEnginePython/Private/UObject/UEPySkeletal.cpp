...
PyObject* py_ue_skeletal_mesh_get_max_bone_influences(ue_PyUObject* self, PyObject* args)
{
	ue_py_check(self);
	...
	int32 max_bone_influences = model.Sections[section_index].MaxBoneInfluences;

	return PyLong_FromLong(max_bone_influences);
}
...
PyObject *py_ue_skeletal_mesh_get_num_triangles(ue_PyUObject *self, PyObject * args)
{
	...
	FSkeletalMeshLODModel &LODModel = resource->LODModels[lod_index];
	int32 num_triangles = 0;
	for (const FSkelMeshSection & Section : LODModel.Sections)
	{
		num_triangles += Section.NumTriangles;
	}
